...
dbConnection().then((db) => {
	return db.dropDatabase().then(() => {
		...
		return users.getUserByUsername("allisonramos").then((thisUser) => {
			let thisProfile = thisUser.userProfile;
			return items.addItem(thisProfile, "Organic Chem Notes", ["school"], "Notes from my Orgo class", 20.00, "Cash", "07030", "/public/uploads/orgo.png", { minDays: 1, maxDays: 21 }, "available");
		});
	}).then(() => {
